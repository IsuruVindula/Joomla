...
if ($submitted) {
    ...
    if (!isset($hasError)) {
        ...
        if ($myparams['mailFromOpt'] === '1') {
            ...
            $sender = array($myparams['mailFrom'], $myparams['mailFromName']);
        }

        $mail->setSender($sender);

        // Common Sender uses Joomla settings never the form user
        $commonSender = array($myparams['mailFrom'], $myparams['mailFromName']);
        ...
        if ($copyme && $copyme == '1' && $email != '' && $sendAdmin == true) {
            ...
            // Set sender to Joomla settings
            $mail->setSender($commonSender);
